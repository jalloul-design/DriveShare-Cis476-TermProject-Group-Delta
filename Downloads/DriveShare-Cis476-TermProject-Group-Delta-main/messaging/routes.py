...
def pay():
    ...
    if request.method == "POST":
        payee_email = request.form["payee_email"]
    ...
    return render_template("messaging/payment.html", user=user, balance=balance)
...
def history():
    ...
    balance = Balance.get_balance(user)
    return render_template("messaging/history.html", user=user, transactions=transactions, balance=balance)
